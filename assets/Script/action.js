...
function shackAction(time, range) {
 ...
 const createRotationTween = (duration, xRotation, yRotation) => {
  return cc.tween(duration)
    .to(cc.Node.prototype.setRotation, {
      x: this.node.rotationX + xRotation,
      y: this.node.rotationY + yRotation
    }, {rotationX: '+=', rotationY: '+='})
    .action();
};

// Helper function to create a rotation to zero tween
const createZeroRotationTween = (duration) => {
  return cc.tween(duration)
    .to(cc.Node.prototype.setRotation, {x: 0, y: 0})
    .action();
};

// 晃动动作
function rockAction(time, range) {
  let action1 = createRotationTween(time, range, range);
  let action2 = createRotationTween(time, -2 * range, -2 * range);
  let action3 = createRotationTween(time * 0.8, 2 * range * 0.8, 2 * range * 0.8);
  let action6 = createRotationTween(time * 0.6, -2 * range * 0.6, -2 * range * 0.6);
  let action7 = createRotationTween(time * 0.4, 2 * range * 0.4, 2 * range * 0.4);
  let action10 = createZeroRotationTween(time * 0.2);
